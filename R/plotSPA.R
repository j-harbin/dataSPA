...
plotSPA <- function(om=NULL,salary=NULL, which=NULL, id=NULL, funding=NULL, fundingChange=NULL, dataframe=FALSE, year=NULL, endDate=NULL, salaries=NULL, debug=0) {
project_id <- category_display <- project_year_id <- amount <- funding_source_display <- fiscal_year <- project_year_id <- category_type <- NULL

if (is.null(which)) {
stop("must provide a which argument of either 'omBar', 'omPie', 'omAllocation', 'omAllocationGeneral','salaryBar', 'salaryAllocation', 'weekAllocation', 'indeterminate', 'predictSummary", 'predict', 'predictSalary')
}

if (!(which %in% c("omBar", "omPie", "omAllocation", "omAllocationGeneral", "salaryBar", "salaryAllocation","weekAllocation","indeterminate", 'predictSummary', 'predict', 'predictSalary'))) {
...
if (which %in% c("salaryBar", "salaryAllocation","weekAllocation","indeterminate", "predictSummary", "predict", 'predictSalary')) {
if (debug > 0) {
...
if (i == 1) {
xlim <- length(x)+1
plot(x, y, type="o", ylab="Number of Stations", pch=20, col=i+1, ylim=c(min(unlist(unname(totalstations)))-10, max(unlist(unname(totalstations)))+10), xaxt="n", xlab=" ", xlim=c(0,xlim))
...
text(x=x[1:length(years)], y=y[1:length(years)], labels= y[1:length(years)], pos=3, cex=0.5)
text(x=rep((length(years)+1), length(columnnames)+1), y=c(y[length(years)+1], unlist(stationLabels)), labels= round(c(y[length(years)+1], unlist(stationLabels)),0), pos=4, cex=0.5)
legend("bottomleft", labels, col=1:(length(x)+1), pch=20, cex=0.7)
...
plot(1:length(ts), unlist(unname(ts)), type="o", pch=20, axes=FALSE, xlab="", ylab="", ylim=c(0, (max(unlist(unname(ts)))*1.15)), col="blue")
text(x=1:length(ts), y=unlist(unname(ts)), labels= unlist(unname(ts)), pos=3, cex=0.5)
...
DFSAL <- data.frame(matrix(NA, nrow = length(salnamesFunding), ncol = ncol))

ly <- gsub("^[^-]*-\\s*([^.]+).*","\\1",salyears[length(salyears)]) # Getting last number of last year (ie. 2023-2024)
newyear <- paste0(ly, "-", as.numeric(ly)+1)
ny2<- as.numeric(gsub("^[^-]*-\\s*([^.]+).*","\\1",newyear))
newyear2 <- paste0(ny2, "-", ny2+1)
ny3 <- as.numeric(gsub("^[^-]*-\\s*([^.]+).*","\\1",newyear2))
...
if (is.null(salaries)) {
  stop("must load salaries data frame by using data(salaries) and set salaries=salaries in plotSPA() argument")
}
s <- salaries[which(grepl(unique(fundingLevel)[[i]], salaries$`Level and Step`)),]
eyears <- as.numeric(sub('.* ', '', s$`Level and Step`)) # Extract everything after space to get year
EY <- sort(unique(as.numeric(eyears)))
for (j in seq_along(EY)) {
#message("i = ",i, " and j =", j)
soi[[i]][j] <- mean(s[which(eyears %in% EY[j]),]$`Annual Salary`, na.rm=TRUE)
}
}
...
dfROI <- data.frame(matrix(" ", nrow = length(salnamesFunding), ncol = length(salyears)+3))
...
# Fill in next values
#browser()
value <- salaryKeep[which(salaryKeep$fiscal_year == salyears[length(salyears)]),] # Look at last
new <- value
...
newAmount <- unname(unlist(roi[which(names(roi) == new$level_display[i])])) # Find ROI for level
new$median_salary[i] <- value$median_salary[i] + newAmount # Update calculations
new$salary_per_week[i] <- new$median_salary[i]/52
new$amount_week[i] <- new$salary_per_week[i]*new$duration_weeks[i]
new$amount_overtime[i] <- (new$salary_per_week[i]/37.5)*new$overtime_hours[i]
new$amount_total[i] <- ifelse(value$overtime_hours[i] == 0, new$amount_week[i], (new$amount_week[i] + new$amount_overtime[i]))
totalSum <- sum(new$amount_total[which(new$funding_source_display == unique(new$funding_source_display[k]))], na.rm=TRUE)
...
#browser()
par(mfrow=c(1,1))
bp <- barplot(as.matrix(dfROI), col=c(1:length(salnamesFunding)), ylim=c(0,sum(subset(saldf, select=c(paste0(salyears[1])))) + 109000), xlab="Year", las=2, ylab="Amount of Salary Funding ($)")
