...
res <- try(plotSPA(om=index, theme=unique(index$theme), which="omBar"), silent=TRUE)
```

### Choose an om allocation plot {.tabset .tabset-dropdown}

```{r, results="asis", echo=FALSE}
df <- try(plotSPA(om=index, which="omAllocation", theme=unique(index$theme), dataframe = TRUE), silent=TRUE)
headers <- names(df)
...
res <- try(plotSPA(om=index,which="predictOM", theme=theme[i]), silent=TRUE)
```
...
res <- try(plotSPA(om=research1, theme=unique(index$theme), which="omBar"), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=research1, which="omAllocation", theme=unique(index$theme), dataframe = TRUE), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and O&M increased by 2%. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=research1,which="predictOM", theme=theme[i]), silent=TRUE)
}
```
...
res <- try(plotSPA(om=monitor1, theme=unique(index$theme), which="omBar"), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=monitor1, which="omAllocation", theme=unique(index$theme), dataframe = TRUE), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and O&M increased by 2%. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=monitor1,which="predictOM", theme=theme[i]), silent=TRUE)
}
```
...
res <- try(plotSPA(om=assessment1, theme=unique(index$theme), which="omBar"), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=assessment1, which="omAllocation", theme=unique(index$theme), dataframe = TRUE), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and O&M increased by 2%. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=assessment1,which="predictOM", theme=theme[i]), silent=TRUE)
}
```
...
res <- try(plotSPA(om=management1, theme=unique(index$theme), which="omBar"), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=management1, which="omAllocation", theme=unique(index$theme), dataframe = TRUE), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
if (run) {
res <- try(plotSPA(om=management1,which="predictOM", theme=theme[i]), silent=TRUE)
}
```
...
res <- try(plotSPA(om=other1, theme=unique(index$theme), which="omBar"), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=other1, which="omAllocation", theme=unique(index$theme), dataframe = TRUE), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and O&M increased by 2%. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=other1,which="predictOM", theme=theme[i]), silent=TRUE)
}
```
