...
cat("The project IDs included in this theme are: ", LINKS)
cat("\n")
cat("\n")
...
```{r, echo=FALSE, fig.cap="Bar chart showing the amount of O&M money ($) invested into the approved projects in the Maritimes region for this theme. Graph color coded by funding type."}
res <- try(plotSPA(om=index, theme=unique(index$theme), which="omBar", approved=approved), silent=TRUE)
```

### Choose an om allocation plot {.tabset .tabset-dropdown}

```{r, results="asis", echo=FALSE}
df <- try(plotSPA(om=index, which="omAllocation", theme=unique(index$theme), dataframe = TRUE, approved=approved), silent=TRUE)
headers <- names(df)
...
res <- try(plotSPA(om=index,which="predictOM", theme=theme[i], approved=approved), silent=TRUE)
```
...
res <- try(plotSPA(salary=index2, theme=unique(index2$theme), which="salaryBar", approved=approved), silent=TRUE)
```
...
res <- try(plotSPA(om=index, salary=index2, which="predictSalary", theme=theme[i], approved=approved), silent=TRUE)
...
```{r, echo=FALSE, fig.cap="Bar chart showing the amount of O&M money ($) invested into the approved research projects of the Maritimes region for the theme. Graph color coded by funding type as indicated by the legend.", results="asis", comment=NA}
run <- ifelse(("Research" %in% unique(index$activity_type)), TRUE, FALSE)
if (run) {
research1 <- index[which(index$activity_type == "Research"),]
res <- try(plotSPA(om=research1, theme=unique(index$theme), which="omBar", approved=approved), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=research1, which="omAllocation", theme=unique(index$theme), dataframe = TRUE, approved=approved), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and O&M increased by 2%. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=research1,which="predictOM", theme=theme[i], approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(salary=research2, theme=unique(index2$theme), which="salaryBar", approved=approved), silent=TRUE)
}
```
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(salary=research2, which="salaryAllocation", theme=unique(research2$theme), dataframe = TRUE, approved=approved), silent=TRUE)
...
if (run) {
res <- try(plotSPA(om=research1, salary=research2, which="predictSalary", theme=unique(index2$theme), approved=approved), silent=TRUE)
}
```
...
```{r, echo=FALSE, fig.cap="Bar chart showing the amount of O&M money ($) invested into the approved monitoring projects in the Maritimes region for this theme. Graph color coded by funding type as shown in the legend.",results="asis", comment=NA}
run <- ifelse(("Monitoring" %in% unique(index$activity_type)), TRUE, FALSE)
if (run) {
monitor1 <- index[which(index$activity_type == "Monitoring"),]
res <- try(plotSPA(om=monitor1, theme=unique(index$theme), which="omBar", approved=approved), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=monitor1, which="omAllocation", theme=unique(index$theme), dataframe = TRUE, approved=approved), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and O&M increased by 2%. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=monitor1,which="predictOM", theme=theme[i], approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(salary=monitor2, theme=unique(index2$theme), which="salaryBar", approved=approved), silent=TRUE)
}
```
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(salary=monitor2, which="salaryAllocation", theme=unique(monitor2$theme), dataframe = TRUE, approved=approved), silent=TRUE)
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and salary cost continued to increase. The rate of increase is determined using linear regression. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=monitor1, salary=monitor2, which="predictSalary", theme=unique(index2$theme), approved=approved), silent=TRUE)
}
```
...
```{r, echo=FALSE, fig.cap="Bar chart showing the amount of O&M money ($) invested into the approved assessment projects in the Maritimes region for this theme. Graph color coded by funding type as shown by the legend.",results="asis", comment=NA}
run <- ifelse(("Assessment" %in% unique(index$activity_type)), TRUE, FALSE)
if (run) {
assessment1 <- index[which(index$activity_type == "Assessment"),]
res <- try(plotSPA(om=assessment1, theme=unique(index$theme), which="omBar", approved=approved), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=assessment1, which="omAllocation", theme=unique(index$theme), dataframe = TRUE, approved=approved), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and O&M increased by 2%. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=assessment1,which="predictOM", theme=theme[i], approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(salary=assessment2, theme=unique(index2$theme), which="salaryBar", approved=approved), silent=TRUE)
}
```
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(salary=assessment2, which="salaryAllocation", theme=unique(assessment2$theme), dataframe = TRUE, approved=approved), silent=TRUE)
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and salary cost continued to increase. The rate of increase is determined using linear regression. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=assessment1, salary=assessment2, which="predictSalary", theme=unique(index2$theme), approved=approved))
}
```
...
```{r, echo=FALSE, fig.cap="Bar chart showing the amount of O&M money ($) invested into the approved data management projects in the Maritimes region for this theme. Graph color coded by funding type as shown by the legend.",results="asis", comment=NA}
...
res <- try(plotSPA(om=management1, theme=unique(index$theme), which="omBar", approved=approved), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=management1, which="omAllocation", theme=unique(index$theme), dataframe = TRUE, approved=approved), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
if (run) {
res <- try(plotSPA(om=management1,which="predictOM", theme=theme[i], approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(salary=management2, theme=unique(index2$theme), which="salaryBar", approved=approved), silent = TRUE)
}
```
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(salary=management2, which="salaryAllocation", theme=unique(management2$theme), dataframe = TRUE, approved=approved), silent=TRUE)
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and salary cost continued to increase. The rate of increase is determined using linear regression. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=management1, salary=management2, which="predictSalary", theme=unique(index2$theme), approved=approved))
}
```
...
```{r, echo=FALSE, fig.cap="Bar chart showing the amount of O&M money ($) invested into the approved other projects in the Maritimes region for this theme. Graph color coded by funding type as shown by the legend.",results="asis", comment=NA}
run <- ifelse(("Other" %in% unique(index$activity_type)), TRUE, FALSE)
if (run) {
other1 <- index[which(index$activity_type == "Other"),]
res <- try(plotSPA(om=other1, theme=unique(index$theme), which="omBar", approved=approved), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=other1, which="omAllocation", theme=unique(index$theme), dataframe = TRUE, approved=approved), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and O&M increased by 2%. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=other1,which="predictOM", theme=theme[i], approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(salary=other2, theme=unique(index2$theme), which="salaryBar", approved=approved), silent=TRUE)
}
```
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(salary=other2, which="salaryAllocation", theme=unique(other2$theme), dataframe = TRUE, approved=approved), silent=TRUE)
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and salary cost continued to increase. The rate of increase is determined using linear regression. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=other1, salary=other2, which="predictSalary", theme=unique(index2$theme), approved=approved), silent=TRUE)
}
```
