...
```{r, echo=FALSE, fig.cap="Bar chart showing the amount of O&M money ($) invested into the approved projects in the Maritimes region for this theme. Graph color coded by funding type and  show the number of deliverables and milestones per year in blue and red respectively."}
res <- try(plotSPA(om=index, theme=unique(index$theme), which="omBar", status=status), silent=TRUE)
```
...
res <- try(plotSPA(om=index,which="predictOM", theme=theme[i], status=status), silent=TRUE)
```
...
res <- try(plotSPA(salary=index2, theme=unique(index2$theme), which="salaryBar", status=status), silent=TRUE)
```
...
res <- try(plotSPA(om=research1, theme=unique(index$theme), which="omBar", status=status), silent=TRUE)
} else {
...
res <- try(plotSPA(salary=research2, theme=unique(index2$theme), which="salaryBar", status=status), silent=TRUE)
}
```
...
res <- try(plotSPA(om=monitor1, theme=unique(index$theme), which="omBar", status=status), silent=TRUE)
} else {
...
res <- try(plotSPA(salary=monitor2, theme=unique(index2$theme), which="salaryBar", status=status), silent=TRUE)
}
```
...
res <- try(plotSPA(om=assessment1, theme=unique(index$theme), which="omBar", status=status), silent=TRUE)
} else {
...
res <- try(plotSPA(salary=assessment2, theme=unique(index2$theme), which="salaryBar", status=status), silent=TRUE)
}
```
...
res <- try(plotSPA(om=management1, theme=unique(index$theme), which="omBar", status=status), silent=TRUE)
} else {
...
res <- try(plotSPA(salary=management2, theme=unique(index2$theme), which="salaryBar", status=status), silent = TRUE)
}
```
...
res <- try(plotSPA(om=other1, theme=unique(index$theme), which="omBar", status=status), silent=TRUE)
} else {
...
res <- try(plotSPA(salary=other2, theme=unique(index2$theme), which="salaryBar", status=status), silent=TRUE)
}
```
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and salary cost continued to increase. The rate of increase is determined using linear regression. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(salary=other2, which="predictSalary", theme=unique(index2$theme), status=status), silent=TRUE)
}
```
