...
```{r, echo=FALSE, comment=NA, results="asis"}
cat("The project IDs included in this theme include: ", paste0(unique(index$project_id), collapse=","))

```

## Select relevant tab for summaries {.tabset}
...
plotSPA(om=index, theme=theme[i], which="omBar")
```
...
#plotSPA(om=index, theme=theme, which="omAllocation")
```
...
plotSPA(salary=index2, theme=theme[i], which="salaryBar")
```
...
#plotSPA(salary=index2, theme=theme, which="salaryAllocation")
```
