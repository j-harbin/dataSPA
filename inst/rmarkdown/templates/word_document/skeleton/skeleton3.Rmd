...
cat("The project IDs included in this functional group are: ", LINKS)
cat("\n")
cat("\n")
...
```{r, echo=FALSE, fig.cap="Bar chart showing the amount of O&M money ($) invested into the approved projects in the Maritimes region for this functional group. Graph color coded by funding type as shown by the legend."}
res <- try(plotSPA(om=index, functionalGroup=unique(index$functional_group), which="omBar", approved=approved), silent=TRUE)
```

### Choose an om allocation plot {.tabset .tabset-dropdown}

```{r, results="asis", echo=FALSE}
df <- try(plotSPA(om=index, which="omAllocation", functionalGroup =unique(index$functional_group), dataframe = TRUE, approved=approved), silent=TRUE)
headers <- names(df)
...
res <- try(plotSPA(om=index,which="predictOM", functionalGroup=functionalGroup[i], approved=approved), silent=TRUE)
```
...
res <- try(plotSPA(salary=index2, functionalGroup=unique(index2$functional_group), which="salaryBar", approved=approved), silent=TRUE)
```
...
```{r, results="asis", echo=FALSE}
df <- try(plotSPA(salary=index2, which="salaryAllocation", functionalGroup=unique(index2$functional_group), dataframe = TRUE, approved=approved), silent=TRUE)
...
res <- try(plotSPA(om=index, salary=index2, which="predictSalary", functionalGroup=functionalGroup[i], approved=approved), silent=TRUE)
...
res <- try(plotSPA(om=research1, functionalGroup=unique(index$functional_group), which="omBar", approved=approved), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=research1, which="omAllocation", functionalGroup=unique(index$functional_group), dataframe = TRUE, approved=approved), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and O&M increased by 2%. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=research1,which="predictOM", functionalGroup=functionalGroup[i], approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(salary=research2, functionalGroup=unique(index2$functional_group), which="salaryBar", approved=approved), silent=TRUE)
}
```
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(salary=research2, which="salaryAllocation", functionalGroup=unique(research2$functional_group), dataframe = TRUE, approved=approved), silent=TRUE)
...
if (run) {
res <- try(plotSPA(om=research1, salary=research2, which="predictSalary", functionalGroup=unique(index2$functional_group), approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(om=monitor1, functionalGroup=unique(index$functional_group), which="omBar", approved=approved), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=monitor1, which="omAllocation", functionalGroup=unique(index$functional_group), dataframe = TRUE, approved=approved), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and O&M increased by 2%. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=monitor1,which="predictOM", functionalGroup=functionalGroup[i], approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(salary=monitor2, functionalGroup=unique(index2$functional_group), which="salaryBar", approved=approved), silent=TRUE)
}
```
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(salary=monitor2, which="salaryAllocation", functionalGroup=unique(monitor2$functional_group), dataframe = TRUE, approved=approved), silent=TRUE)
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and salary cost continued to increase. The rate of increase is determined using linear regression. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=monitor1, salary=monitor2, which="predictSalary", functionalGroup=unique(index2$functional_group), approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(om=assessment1, functionalGroup=unique(index$functional_group), which="omBar", approved=approved), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=assessment1, which="omAllocation", functionalGroup=unique(index$functional_group), dataframe = TRUE, approved=approved), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and O&M increased by 2%. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=assessment1,which="predictOM", functionalGroup=functionalGroup[i], approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(salary=assessment2, functionalGroup=unique(index2$functional_group), which="salaryBar", approved=approved), silent=TRUE)
}
```
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(salary=assessment2, which="salaryAllocation", functionalGroup=unique(assessment2$functional_group), dataframe = TRUE, approved=approved), silent=TRUE)
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and salary cost continued to increase. The rate of increase is determined using linear regression. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=assessment1, salary=assessment2, which="predictSalary", functionalGroup=unique(index2$functional_group), approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(om=management1, functionalGroup=unique(index$functional_group), which="omBar", approved=approved), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=management1, which="omAllocation", functionalGroup=unique(index$functional_group), dataframe = TRUE,approved=approved), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
if (run) {
res <- try(plotSPA(om=management1,which="predictOM", functionalGroup=functionalGroup[i], approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(salary=management2, functionalGroup=unique(index2$functional_group), which="salaryBar", approved=approved), silent = TRUE)
}
```
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(salary=management2, which="salaryAllocation", functionalGroup=unique(management2$functional_group), dataframe = TRUE, approved=approved), silent=TRUE)
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and salary cost continued to increase. The rate of increase is determined using linear regression. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=management1, salary=management2, which="predictSalary", functionalGroup=unique(index2$functional_group), approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(om=other1, functionalGroup=unique(index$functional_group), which="omBar", approved=approved), silent=TRUE)
} else {
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(om=other1, which="omAllocation", functionalGroup=unique(index$functional_group), dataframe = TRUE, approved=approved), silent=TRUE)
headers <- names(df)
for (j in seq_along(headers)) {
     cat("#### ", headers[j], "\n\n",
         knitr::knit_child("plot.Rmd", quiet = TRUE), "\n\n")
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and O&M increased by 2%. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=other1,which="predictOM", functionalGroup=functionalGroup[i], approved=approved), silent=TRUE)
}
```
...
res <- try(plotSPA(salary=other2, functionalGroup=unique(index2$functional_group), which="salaryBar", approved=approved), silent=TRUE)
}
```
...
```{r, results="asis", echo=FALSE}
if (run) {
df <- try(plotSPA(salary=other2, which="salaryAllocation", functionalGroup=unique(other2$functional_group), dataframe = TRUE, approved=approved), silent=TRUE)
...
```{r, echo=FALSE, fig.cap="Bar graph showing gap in funding (red) if funding stayed the same and salary cost continued to increase. The rate of increase is determined using linear regression. Actual years are shown on the left of the dashed line, and predicted years are on the right of the dashed line. Total gap amount rounded to the nearest dollar shown in red text."}
if (run) {
res <- try(plotSPA(om=other1, salary=other2, which="predictSalary", functionalGroup=unique(index2$functional_group), approved=approved), silent=TRUE)
}
```
