...
res <- try(plotSPA(salary=index2, functionalGroup=unique(index2$functional_group), which="salaryBar", status=status), silent=TRUE)
```
...
cat("### Choose a project id to view associated deliverables {.tabset .tabset-dropdown}")
```

```{r, results="asis", echo=FALSE}
headers <- sort(unique(index$project_id))
for (j in seq_along(headers)) {
      cat("#### ", headers[j], "\n\n"
