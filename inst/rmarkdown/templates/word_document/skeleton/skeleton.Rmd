...
SG <-  try(plotSPA(salary=index2,which="predictSalary", theme=unique(index$theme), dataframe=TRUE), silent=TRUE)
...
try(plotSPA(om=index, id=ID, which="omBar"), silent=TRUE)
```
...
res <- try(plotSPA(om=index, id=ID, which="omAllocation"), silent=TRUE)
```
...
plotSPA(om=index, which="predictOM", id=ID)#, silent=TRUE)
```
...
#browser()
try(plotSPA(salary=index2, id=ID, which="salaryBar"), silent=TRUE)
```
...
res <- try(plotSPA(om=index, salary=index2, id=ID, which="indeterminate"), silent=TRUE)
```
...
try(plotSPA(salary=index2, which="predictSalary", id=ID), silent=TRUE)
```
