...
8. The salaries are based on time planned on these projects. The actual time spent may have varied.
...
res <- try(plotSPA(om=index, id=ID, which="omBar"), silent=TRUE)
```
...
res <- try(plotSPA(om=index, salary=index2, which="predictOM", id=ID), silent=TRUE)

```


```{r, echo=FALSE, fig.cap="Figure 4: Bar graph representing amount of salary money ($) planned to spent on the project using the median values of a specific job classification. Graph color coded by funding type indicated by the legend."}
res <- try(plotSPA(salary=index2, id=ID, which="salaryBar"), silent=TRUE)
```
...
res <- try(plotSPA(om=index, salary=index2, which="predictSalary", id=ID), silent=TRUE)
