---
title: Specific Project Reports
author: " "
output:
  html_document:
    theme: flatly
...
path <- "//dcnsbiona01a/BIODataSVC/IN/MSP/PowerBI-Projects/dataSPA/outputs/reports/"

ids <- str_extract(list.files(path), "[^.]+")
...
links <- paste0("file:",path, str_extract(list.files(path), "[^.]+"),".html")
df <- data.frame(matrix(NA, ncol = 3, nrow = length(ids)))
names(df) <- c("Title", "Project ID", "PPT")
df$Title <- titles

df$`Project ID` <- paste0('<a href="',links,'">',ids,'</a>')
